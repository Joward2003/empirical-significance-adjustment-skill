...
def main() -> None:
    ...
    # Flags are screening signals, not approval to run a specification.  Keep
    # this mapping deliberately narrow: method-specific inputs and independent
    # evidence must still be supplied by the researcher before execution.
    selected = []
    pending = []
    ...
    selected = [
        method_id
        for method_id in dict.fromkeys(selected)
        if method_id in index and index[method_id]['adjustment_level'] in allowed
    ]
    ...
    for order, method_id in enumerate(selected, 1):
        ...
        restrictions = '；'.join(method['use_restrictions'])
        lines.append(
            f"| {order} | `{method_id}` | {method['adjustment_level']} | "
            f"{method['title_zh']} | {method['purpose']} | {restrictions} |"
        )
    ...
    if pending:
        ...
        lines.append('- 无。仍须在执行前记录每项候选方法的诊断证据与批准人。')

    lines += ['', '## 代码草案', '']
    for method_id in selected:
        method = index[method_id]
        lines += [
            f"### {method_id}｜{method['title_zh']}",
            '```stata',
            method['stata_code'],
            '```',
            '',
